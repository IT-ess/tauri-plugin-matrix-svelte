...
class NotificationsPlugin: Plugin, UNUserNotificationCenterDelegate {
    ...
    @objc private func handleRemoteNotificationRegistration(_ notification: Notification) {
        ...
        if let deviceToken = notification.object as? Data {
            self.application(
                UIApplication.shared, didRegisterForRemoteNotificationsWithDeviceToken: deviceToken)
        }
    }
    ...
    @objc private func handleRemoteNotificationRegistrationError(_ notification: Notification) {
        ...
        if let error = notification.object as? Error {
            self.application(
                UIApplication.shared, didFailToRegisterForRemoteNotificationsWithError: error)
        }
    }
            ...
            DispatchQueue.main.async {
                ...
                case .denied:
                    ...
                    let result = NotificationRegistrationResult(
                        ...
                        token: nil,
                        error:
                            "Push notifications are not authorized. Please enable them in Settings."
                    )
    ...
    private func proceedWithRegistration(_ invoke: Invoke) {
        ...
        Logger.debug(
            "NotificationsPlugin: Application delegate is: \(String(describing: UIApplication.shared.delegate))"
        )
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 10.0) { [weak self] in
            ...
            Logger.debug(
                "NotificationsPlugin: Application delegate at timeout is: \(String(describing: UIApplication.shared.delegate))"
            )
            ...
            let result = NotificationRegistrationResult(
                ...
                token: nil,
                error:
                    "Registration timed out. Please check your internet connection and provisioning profile."
            )
...
extension NotificationsPlugin: UIApplicationDelegate {
    ...
    public func application(
        ...
        Logger.error(
            "NotificationsPlugin: FAILURE - Registration failed with error: \(error.localizedDescription)"
        )
...
@_cdecl("init_plugin_matrix_svelte")

...
use crate::{keychain::save_session, utils::fs::get_app_dir_or_create_it};
...
pub async fn get_client_from_new_session<R: Runtime>(
    ...
    let client_initial_state =
        login_and_persist_session(&app_handle, &matrix_config, app_data_dir).await?;
...
async fn login_and_persist_session<R: Runtime>(
    app_handle: &AppHandle<R>,
    ...
    save_session(app_handle, serialized)?;

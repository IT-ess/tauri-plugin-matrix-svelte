...
use super::utils::{get_or_fetch_event_sender, trim_start_html_whitespace};
        ...
        MessageType::Text(text) => text
            ...
            .and_then(|fb| {
                (fb.format == MessageFormat::Html).then(|| {
                    crate::matrix::utils::linkify(trim_start_html_whitespace(&fb.body), true)
                        .to_string()
                })
            })
            .unwrap_or_else(|| match crate::matrix::utils::linkify(&text.body, false) {

...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // plugin's FCM service loads this library (`SILENT_PUSH_LIB`
            // manifest meta-data) and dispatches every one — warm or killed —
...
pub extern "system" fn Java_com_matrix_svelte_client_MainActivity_initNdkContext(
    ...
    // Shared with the silent-push init hook so `ndk_context` is initialized at
    // most once per process. The FCM service runs in the same process and
    // Android may reuse a push process to launch this activity; using one
    // guard for both entry points avoids `ndk_context`'s double-init panic.
    // See `push_handler`.
    push_handler::ensure_ndk_context(&mut env, &context);
}

// The one silent-push handler for both mobile platforms (iOS NSE dlsym entry +
// Android FCM JNI entry, both generated by `silent_push_handler!`), plus the
// Android init hook replaying process-wide setup on cold starts. See the
// module docs.
#[cfg(any(target_os = "android", target_os = "ios"))]
mod push_handler;

// Platform-agnostic fetch/format helpers shared by the silent-push handler.
#[cfg(any(target_os = "android", target_os = "ios"))]
mod push_shared;
